...
int main(int argc, char* argv[]) {
    ...
    {
        std::cout << "set socket to nonblocking error." << std::endl;
        close(clientfd);
        return -1;
    }
    ...
    for (;;)
    {
        ...
        else if (ret == -1)
        {
            ...
            else
            {
                // 真的出错了
                close(clientfd);
                std::cout << "connect ret < 0, errno: " << errno << std::endl;
                return -1;
            }
        }
    }
    ...
    {
        close(clientfd);
        std::cout << "[poll] connect to connect error." << std::endl;
        return -1;
    }
